...
- For Railway, set `BACKEND_BASE_URL` to the backend service public domain and `NEXT_PUBLIC_BACKEND_SWAGGER_URL` to the same backend public docs URL. This keeps a fresh Railway setup simple and avoids private-network port wiring mistakes. The copy-paste template lives at [RAILWAY_VARIABLES_FRONTEND.env](/C:/Users/Kidus/Documents/sms-platform-backend/RAILWAY_VARIABLES_FRONTEND.env).
